...
function Preloader({ reducedMotion = false }) {
  ...
  useEffect(() => {
    ...
    const animate = (timestamp) => {
      ...
      const rotationSpeed = reducedMotion ? 0.46 : 3.2
      const headAngle = timestamp * 0.00155 * rotationSpeed
      const trailOffset = 0.7
      const glitterSteps = reducedMotion ? 2 : 6
      const leadPulse = 0.86 + Math.sin(timestamp * 0.018) * 0.14
      const trailPulse = 0.66 + Math.sin(timestamp * 0.022 - 1.2) * 0.12
      ...
      for (let index = 0; index < glitterSteps; index += 1) {
        ...
        const glitterPulse = 0.52 + Math.sin(timestamp * 0.024 - index * 0.8) * 0.24
        const glitterAlpha = Math.max(0.06, 0.34 - index * 0.05 + glitterPulse * 0.22)
